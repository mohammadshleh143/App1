...
public class LoadProductActivity extends FragmentActivity {
	private Context act;
	...
	private static final String LOG_TAG = LoadProductActivity.class
			.getSimpleName();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_main_load_product);
		...
		progressDialog
				.setMessage(getApplicationContext()
						.getResources()
						.getString(
								R.string.app_inventory_processing_download_load_product));
		...
		if (countStockVan == 0) {
			if (GlobalApp.checkInternetConnection(act))
				...
				String message = act
						...
						.getResources()
						.getString(
								R.string.app_inventory_processing_download_load_product_empty);
			...
			try {
				...
				if (oResponse.getString("request_load").length() > 0) {
					...
					for (int i = 0; i < jsonarr.length(); i++) {
						...
						String jumlah_accept = oResponsealue.isNull("jumlah_accept") ? null : oResponsealue
	...
	public void showListLoadProduct() {
		...
		if (productStockVan_list.size() > 0) {
			...
			cAdapter = new ListViewAdapter(this,
					R.layout.list_item_load_product, productStockVan_list);
		...
		public View getView(final int position, View convertView,
			...
			if (row == null) {
				...
				holder.tvKodeProduct = (TextView) row
						.findViewById(R.id.load_product_kode_product);
				holder.tvNamaProduct = (TextView) row
						.findViewById(R.id.load_product_nama_product);
				holder.tvStockVan = (TextView) row
						.findViewById(R.id.load_product_stock_van);
				row.setTag(holder);
			} else {
