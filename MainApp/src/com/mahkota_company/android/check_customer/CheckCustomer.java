package com.mahkota_company.android.check_customer;
...
public class CheckCustomer extends ActionBarActivity implements
	...
	private static final String LOG_TAG = CheckCustomer.class
			.getSimpleName();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_main_customer_uploaded);
		...
		searchCustomer.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence cs, int arg1, int arg2,
				...
				if (cs.toString().length() > 0) {
					...
					ArrayList<Customer> customer_from_db = databaseHandler.getAllCustomerActiveUploaded12BaseOnSearch(
							cs.toString(),
					...
					if (customer_from_db.size() > 0) {
						...
						cAdapter = new ListViewAdapter(CheckCustomer.this,
					...
					ArrayList<Customer> customer_from_db = databaseHandler
							.getAllCustomerActiveUploaded12(Integer
	...
	public void updateContentCustomer() {
		...
		if (levelStaff > 2) {
			...
			//		.parseInt(main_app_staff_id_wilayah));
			customer_from_db = databaseHandler.getAllCustomerActiveUploaded12(Integer
					.parseInt(main_app_staff_id_wilayah));
		} else {
			customer_from_db = databaseHandler.getAllCustomerActiveUploaded12();
		}
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.menu_refresh:
			if (GlobalApp.checkInternetConnection(act)) {
				int countUpload = databaseHandler.getCountCustomerWhereValidAndUpdate();
				if(countUpload == 0){
					new DownloadDataCustomer().execute();
				}else{
					String message = act
	...
	*/
	...
	public void gotoDetailCustomer() {
		Intent i = new Intent(this, DetailEditCustomerUploaded.class);
		startActivity(i);
		finish();
	}
	...
	public void onNavigationDrawerItemSelected(int position) {
		if (mNavigationDrawerFragment != null) {
			if (mNavigationDrawerFragment.getCurrentSelectedPosition() != 11) {
				if (position == 0){
					Intent intenActivity = new Intent(this,
							CustomerActivity.class);
					startActivity(intenActivity);
					finish();
				}else if (position == 1) {
