...
public class AddSalesOrderActivity extends FragmentActivity {
	...
	private final OnClickListener maddSalesOrderButtonOnClickListener = new OnClickListener() {
		...
		public void onClick(View arg0) {
			...
			case R.id.activity_sales_order_btn_save:
				if(detailSalesOrderList.equals(null)){
					String msg = getApplicationContext().getResources().getString(
							R.string.MSG_DLG_LABEL_SAVE_ORDER_EMPTY);
					showCustomDialogSaveSuccess(msg);

				}else{
					showConfirmationSaveOrder();
				}
	...
	protected void showConfirmationSaveOrder() {
		String msg = getApplicationContext().getResources().getString(
				R.string.MSG_DLG_LABEL_SAVE_ORDER);
		final AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(
				act);
		alertDialogBuilder
				.setMessage(msg +" "+ etNamaCustomer.getText().toString()+" ?")
				.setCancelable(false)
				.setNegativeButton(
	...
	public void gotoAddSalesOrder() {
		Intent i = new Intent(this, AddSalesOrderActivity.class);
		startActivity(i);
		finish();
	}
